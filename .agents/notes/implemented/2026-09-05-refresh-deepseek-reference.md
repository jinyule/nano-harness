...
- 预推送核对执行 `git fetch origin main` 与 `scripts/change-scope.sh origin/main HEAD`：base 为 `eebb248627f61e38b85d21c3fa0dd4f7c07ab685`，HEAD 为 `43527013edf2cdb95dc404c82dc3b041de843085`。PR #5 已 squash 合并，`git diff origin/main HEAD` 为空，两者 tree 相同；三点历史差异中的核心 harness 已在主线，不是本次新增范围。工作树没有产品 Go、依赖或工具版本变更，因此复用上述仍有效的统一门禁与 release 证据；Note 格式、submodule 和 whitespace 复核通过。未提交或推送。
